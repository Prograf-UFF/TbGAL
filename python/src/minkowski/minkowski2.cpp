...
#include "../../../cpp/include/tbgal/assuming_Minkowski2.hpp"
...
namespace py_tbgal {
    ...
    auto py_vector(python::tuple args, python::dict kwargs) {
        ...
        auto a = tbgal::Minkowski2::vector(args_as_container.begin(), args_as_container.end());
        return a;
    }

    auto py_euclidean_vector(python::tuple args, python::dict kwargs) {
        std::vector<std::double_t> args_as_container = py_list_to_std_vector<std::double_t>(args);
        auto a = tbgal::Minkowski2::euclidean_vector(args_as_container.begin(), args_as_container.end());
        return a;
    }

    auto py_point(python::tuple args, python::dict kwargs) {
        std::vector<std::double_t> args_as_container = py_list_to_std_vector<std::double_t>(args);
        auto a = tbgal::Minkowski2::point(args_as_container.begin(), args_as_container.end());
        return a;
    }

    auto ep() {
        return tbgal::Minkowski2::ep;
    }

    auto em() {
        return tbgal::Minkowski2::em;
    }
    
    BOOST_PYTHON_MODULE(minkowski2) {

        using namespace tbgal::Minkowski2;

        using OP = tbgal::OuterProduct<tbgal::MinkowskiMetricSpace<2>>;
        using GP = tbgal::GeometricProduct<tbgal::MinkowskiMetricSpace<2>>;
        ...
        python::def("e", &tbgal::Minkowski2::e);
        python::def("ep", &ep);
        python::def("em", &em);
