...
namespace py_tbgal {
    ...
    auto py_vector(python::tuple args, python::dict kwargs) {
        ...
        auto a = tbgal::Conformal1::vector(args_as_container.begin(), args_as_container.end());
        return a;
    }

    auto py_euclidean_vector(python::tuple args, python::dict kwargs) {
        std::vector<std::double_t> args_as_container = py_list_to_std_vector<std::double_t>(args);
        auto a = tbgal::Conformal1::euclidean_vector(args_as_container.begin(), args_as_container.end());
        return a;
    }

    auto py_point(python::tuple args, python::dict kwargs) {
        std::vector<std::double_t> args_as_container = py_list_to_std_vector<std::double_t>(args);
        auto a = tbgal::Conformal1::point(args_as_container.begin(), args_as_container.end());
        return a;
    }

    auto no() {
        return tbgal::Conformal1::no;
    }

    auto ni() {
        return tbgal::Conformal1::ni;
    }


    BOOST_PYTHON_MODULE(conformal1) {
        using namespace tbgal::Conformal1;

        using OP = tbgal::OuterProduct<tbgal::ConformalMetricSpace<1>>;
        using GP = tbgal::GeometricProduct<tbgal::ConformalMetricSpace<1>>;
        ...
        python::def("e", &tbgal::Conformal1::e);
        python::def("no", &no);
        python::def("ni", &ni);
