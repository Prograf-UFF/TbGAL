...
namespace py_tbgal {
    ...
    auto py_euclidean_vector(python::tuple args, python::dict kwargs) {
        std::vector<std::double_t> args_as_container = py_list_to_std_vector<std::double_t>(args);
        auto a = tbgal::HomogeneousD::euclidean_vector(args_as_container.begin(), args_as_container.end());
        return a;
    }

    auto py_direction(python::tuple args, python::dict kwargs) {
        std::vector<std::double_t> args_as_container = py_list_to_std_vector<std::double_t>(args);
        auto a = tbgal::HomogeneousD::direction(args_as_container.begin(), args_as_container.end());
        return a;
    }

    auto py_point(python::tuple args, python::dict kwargs) {
        std::vector<std::double_t> args_as_container = py_list_to_std_vector<std::double_t>(args);
        auto a = tbgal::HomogeneousD::point(args_as_container.begin(), args_as_container.end());
        return a;
    }

    void set_base_space_dimensions(const int &dims) {
        tbgal::HomogeneousD::SPACE.set_base_space_dimensions(dims);
    }

    BOOST_PYTHON_MODULE(homogeneousD) {

        tbgal::HomogeneousD::SPACE = HomogeneousMetricSpace<Dynamic, Dynamic>();
        ...
        python::def("e", &tbgal::HomogeneousD::e);
