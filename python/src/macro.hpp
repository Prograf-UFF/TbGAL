...
#define PY_TBGAL_EXPOSE_UTILS() \
    ...
    py::def("vector", py::raw_function(+[](py::tuple const &args, py::dict const &) { return vector(py::stl_input_iterator<DefaultScalarType>(args), py::stl_input_iterator<DefaultScalarType>()); }))

#define PY_TBGAL_EXPOSE_FUNCTION(NAME, FUNCTION) \
    py::def(NAME, &FUNCTION)

#define PY_TBGAL_EXPOSE_VARIABLE(NAME, INSTANCE) \
    py::scope().attr(NAME) = py::object(py::ptr(&INSTANCE))
