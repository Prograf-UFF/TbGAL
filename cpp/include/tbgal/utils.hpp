...
namespace tbgal {
    ...
    namespace detail {
        ...
        constexpr decltype(auto) make_vector(MetricSpaceType const &space, ScalarTypes &&... coords) noexcept {
        ...
        constexpr decltype(auto) make_vector_using_iterator(MetricSpaceType const &space, IteratorType begin, IteratorType end, ExtraScalarTypes &&... extra_coords) noexcept {
            ...
            using ResultingFactoringProductType = OuterProduct<MetricSpaceType>;
            using ResultingFactoredMultivectorType = FactoredMultivector<ResultingScalarType, ResultingFactoringProductType>;
            assert(space.dimensions() == (std::distance(begin, end) + sizeof...(extra_coords)));
            auto input = detail::evaluate(detail::from_actual_to_signed_metric(space, detail::fill_column_matrix_using_iterator<MetricSpaceType::DimensionsAtCompileTime, MetricSpaceType::MaxDimensionsAtCompileTime>(begin, end, std::move(extra_coords)...)));
            auto qr_tuple = detail::qr_orthogonal_matrix(input);
    ...
    decltype(auto) vector(MetricSpaceType const &space, ScalarTypes &&... coords) noexcept {
        return detail::make_vector(space, std::move(coords)...);
    }
    ...
    decltype(auto) vector(MetricSpaceType const &space, IteratorType begin, IteratorType end) noexcept {
        return detail::make_vector_using_iterator(space, begin, end);
    }
