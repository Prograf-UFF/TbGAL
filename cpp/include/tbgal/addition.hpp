...
namespace tbgal {
    ...
    constexpr decltype(auto) ADD(FactoredMultivector<FirstFactoringProductType, FirstSquareMatrixType> const &arg1, SecondScalarType const &arg2) noexcept {
        if (arg1.factors_count() == 0) {
    ...
    template<typename FirstScalarType, typename SecondFactoringProductType, typename SecondSquareMatrixType, typename = std::enable_if_t<!is_multivector_v<FirstScalarType> > >
    constexpr decltype(auto) ADD(FirstScalarType const &arg1, FactoredMultivector<SecondFactoringProductType, SecondSquareMatrixType> const &arg2) noexcept {
