...
namespace tbgal {
    ...
    constexpr decltype(auto) reverse(FactoredMultivector<ScalarType, GeometricProduct<MetricSpaceType> > const &arg) {
        ...
        return ResultingFactoredMultivectorType(arg.space_ptr(), arg.scalar(), detail::reverse_columns(arg.factors_in_signed_metric()));
    }

    template<typename ScalarType, typename MetricSpaceType>
    constexpr decltype(auto) reverse(FactoredMultivector<ScalarType, OuterProduct<MetricSpaceType> > const &arg) {
        using ResultingScalarType = ScalarType;
    ...
    constexpr Type reverse(Type const &arg) {
        return arg;
    }
